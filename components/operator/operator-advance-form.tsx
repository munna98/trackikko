...
export const operatorAdvanceSchema = z.object({
...
export type OperatorAdvanceFormValues = z.infer<typeof operatorAdvanceSchema>

type Step = 1 | 2 | 3

const STEP_META: Record<Step, { label: string; progress: number }> = {
  1: { label: 'Select Party', progress: 50 },
  ...
  return (
    <div className="flex flex-col items-center text-center space-y-5 py-4">
      ...
      <div className="space-y-1">
        <p className="text-xl font-bold text-foreground">Advance Recorded!</p>
        <p className="text-sm text-muted-foreground">{info.partyName}</p>
      </div>
...
export function OperatorAdvanceForm({ parties, accounts }: OperatorAdvanceFormProps) {
  ...
  async function handleNextStep() {
    const valid = await form.trigger(['partyId', 'date'])
    if (!valid) return
    setStep(2)
  }
  ...
  async function onSubmit(values: OperatorAdvanceFormValues) {
    if (!selectedParty) return

    const res = await fetch(`/api/parties/${values.partyId}/advances`, {
      method: 'POST',
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            {step === 1 && (
              <>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Party <span className="text-destructive">*</span></FormLabel>
            ...
            {step === 2 && (
              <>
                {selectedParty && (
                  <div className="rounded-xl bg-muted px-4 py-2.5 flex items-center text-sm text-muted-foreground">
                    <span className="font-medium text-foreground truncate">{selectedParty.name}</span>
                  </div>
                )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Received In Account <span className="text-destructive">*</span></FormLabel>
                      <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger className="w-full min-h-[48px]">
